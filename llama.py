...
class LlamaModel(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.output_hidden_layer = args.output_hidden_layer
    ...
    def __call__(
        ...
        if self.output_hidden_states and self.output_hidden_layer == 0:
            # print("Appending shape", h.shape)
        ...
        # make mlx array of hidden states
...
class Model(nn.Module):
    ...
    def __call__(
        ...
        model_output = self.model(inputs, cache)
        
        if self.model.output_hidden_states:
            out, hidden_states = model_output
        else:
            out = model_output
            hidden_states = None
